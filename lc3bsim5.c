...
/***************************************************************/
int GetIRD1(int *x)           { return(x[IRD1]); }
int GetIRD0(int *x)           { return(x[IRD0]); }
...
int GetSETM(int *x)             { return(x[SETM]);}
...
void eval_micro_sequencer() {
  ...
  int PROT, UNALI, UNKWNOP, PGFAULT;

  if( GetCOND(CURRENT_LATCHES.MICROINSTRUCTION) == 4){
    if( ((CURRENT_LATCHES.IR >> 12) & 0x000F) != 15
        && (CURRENT_LATCHES.PSR >> 15)
        && ((CURRENT_LATCHES.MDR & 0x0008) >> 3 == 0)) {
        PROT = 1;
    }
    else PROT = 0;


    if( ((CURRENT_LATCHES.STATE_NUMBER == 7) || CURRENT_LATCHES.STATE_NUMBER == 6) & (NEXT_LATCHES.VA&0x0001) )
        UNALI = 1;
    else
        UNALI = 0;
    ...
    if(CURRENT_LATCHES.MDR & PTE_VALID_MASK)
        PGFAULT = 0;
    else
        PGFAULT = 1;
    
    if(PGFAULT || UNKWNOP || UNALI || PROT) CURRENT_LATCHES.EXC = 1;
  ...
  /* LOOKUP TABLE for JSTATE */  
  if     (CURRENT_LATCHES.STATE_NUMBER == 18)  JState = 33;
  else if(CURRENT_LATCHES.STATE_NUMBER == 19)  JState = 33;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  8)  JState = 60;
  else if(CURRENT_LATCHES.STATE_NUMBER == 41)  JState = 52;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  2)  JState = 29;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  6)  JState = 25;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  7)  JState = 23;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  3)  JState = 24;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  38)  JState = 40;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  43)  JState = 37;
  else if(CURRENT_LATCHES.STATE_NUMBER ==  39)  JState = 45;
  ...
  if(CURRENT_LATCHES.STATE_NUMBER == 7 || CURRENT_LATCHES.STATE_NUMBER == 3){
    modified = 1;
            ...
            CURRENT_LATCHES.STATE_NUMBER == 6 || 
            CURRENT_LATCHES.STATE_NUMBER == 41 || 
            CURRENT_LATCHES.STATE_NUMBER == 18 || 
            CURRENT_LATCHES.STATE_NUMBER == 18 || 
            CURRENT_LATCHES.STATE_NUMBER == 38 || 
            CURRENT_LATCHES.STATE_NUMBER == 43 || 
            CURRENT_LATCHES.STATE_NUMBER == 39 ){
    modified = 0;
  }

  if( GetIRD1(CURRENT_LATCHES.MICROINSTRUCTION) ){
    NEXT_LATCHES.STATE_NUMBER = JState;
  }
  
  else if( GetIRD0(CURRENT_LATCHES.MICROINSTRUCTION) ){
...
//     printf("MDR <- (PSR) 0x%.4x @ CYCLE %i\n", CURRENT_LATCHES.PSR, CYCLE_COUNT);
//   }
//   if(CURRENT_LATCHES.STATE_NUMBER == 54){
//     printf("PSR <- (MDR) 0x%.4x @ CYCLE %i\n", CURRENT_LATCHES.MDR, CYCLE_COUNT);
//   }

//   if(CURRENT_LATCHES.STATE_NUMBER == 26){
//     printf("RTIE DONE @ CYCLE %i(after this one, this is cycle 26)\n", CYCLE_COUNT);
//   }
...
void eval_bus_drivers() {
  ...
  if( GetGATE_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION) ){
    ...
    drivers = gatePTEADDR;
  } else if( GetGATE_PFN(CURRENT_LATCHES.MICROINSTRUCTION) ){
    drivers = gatePFN;
...
void drive_bus() {
  ...
  else if(drivers == gatePTBR){
    BUS = CURRENT_LATCHES.PTBR;
  }

  else if(drivers == gateVA || drivers == gatePFN){
    BUS = CURRENT_LATCHES.VA & 0x01FF;
    BUS |= ((CURRENT_LATCHES.PFN << 9) & 0x3E00);
  }

  else if(drivers == gatePTEADDR){
    int value = 0;
    value = (CURRENT_LATCHES.VA >> 9) & 0x007F;
    value = value << 1;
    value |= (CURRENT_LATCHES.PTBR & 0xFF00);
    BUS = value;
  }
...
void latch_datapath_values() {
  ...
  if( GetLD_IR(CURRENT_LATCHES.MICROINSTRUCTION) ) {
    ...
    // switch (inst){
        ...
        // case 5 : printf("AND\n"); break;
        // case 0 : printf("BR\n"); break;
        // case 12 : printf("JMP @ CYCLE: %i\n", CYCLE_COUNT); break; break;
        // case 4 : printf("JSR\n"); break;
        // case 2 : printf("LDB @ CYCLE: %i\n", CYCLE_COUNT); break;
        // case 6 : printf("LDW\n"); break;
        // case 14 : printf("LEA\n"); break;
        // case 8 : printf("RTI @ CYCLE: %i\n", CYCLE_COUNT);break;
        // case 13 : printf("SHF\n"); break;
        // case 3 : printf("STB\n"); break;
        // case 7 : printf("STW\n"); break;
        // case 15 : printf("TRAP\n"); break;
  ...
  if( GetLD_PFN(CURRENT_LATCHES.MICROINSTRUCTION)){
    NEXT_LATCHES.PFN = ((CURRENT_LATCHES.MDR >> 9) & 0x001F);
  }

  if (GetSETM(CURRENT_LATCHES.MICROINSTRUCTION)){
    NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR | ((modified << 1) & 0x0002);
  }
  if (GetSETR(CURRENT_LATCHES.MICROINSTRUCTION)){
